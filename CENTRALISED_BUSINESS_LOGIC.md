...
### Before writing a new function (mandatory)
1. Search first, write second. Search by:
   - keyword and partial name;
   - function signature (arg shape, return shape);
   - behavioural match (what it does, not just what it is called).
2. Search locations in order:
   - canonical shared modules (e.g. `packages/validation`, `packages/security`, `lib/rules`);
   - the service layer and domain layer;
   - neighbouring files in the same package or module.
3. If a near-match exists, prefer parameterising the existing function over creating a variant.
4. If reuse is genuinely impossible, record the decision in the plan's "Pre-implementation gates → Prior-art and reuse check" section (see `PLANNING.md`), and note it in the PR's "Reuse and alignment" section (see `DEVELOPMENT_PROCESS.md`).
5. Run the duplication scan defined in `CI_GATES.md` locally before opening a PR.
